...
import xyz.devnote.orderfoodonline.load

class FoodAdapter(private val items : List<FoodViewModel.Food> = listOf(),
                  private val listener : Listener) : RecyclerView.Adapter<FoodAdapter.ViewHolder>() {
    ...
    inner class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        fun binding(item : FoodViewModel.Food) {
            ...
            itemView.image_product.setOnClickListener {
                listener.onProductClickListener(item)
            }
        }
    }

    interface Listener {
        fun onProductClickListener(item : FoodViewModel.Food)
    }
}